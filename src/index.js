...
const Owner_Discord = "703966523647197275"; // ID Discord คนที่จะพิมพ์ได้
const Room_Discord = "1110560774272860160"; // ID ห้อง Discord ที่จะใช้งาน
const Role = "1117439561627340881"; // ID Role Discord ที่จะใช้งาน
...
client.once(Events.ClientReady, (readyClient) => {
  console.log(``);
  console.log(
    `${ANSI_WHITE}[${ANSI_BLUE}Allright Town${ANSI_WHITE}]${ANSI_GREEN}#########################################`
  );
  console.log(
    `${ANSI_WHITE}[${ANSI_BLUE}Allright Town${ANSI_WHITE}] ${ANSI_CYAN}Thank you for purchasing XD STUDIO`
  );
  console.log(
    `${ANSI_WHITE}[${ANSI_BLUE}Allright Town${ANSI_WHITE}]         ${ANSI_PURPLE}Allright Town           `
  );
  console.log(
    `${ANSI_WHITE}[${ANSI_BLUE}Allright Town${ANSI_WHITE}]${ANSI_GREEN}#########################################`
  );
...
client.on("messageCreate", async (message) => {
  if (
    message.author.bot ||
    message.channelId != Room_Discord ||
    message.author.id != Owner_Discord
  )
    return;
  ...
  const RoomMaster = client.channels.cache.get(Room_Discord);
    ...
    embeds: [
      {
        ...
        description:
        `กดเพื่อรับยศ <@&${Role}> ยศนี้จะมีสิทธิพิเศษต่างๆมากมาย เฉพาะผู้เล่นที่มียศนี้เท่านั้น\n\n**Fanpage : [ALLRIGHT TOWN](https://www.facebook.com/AllrightTown)**`,
        color: 529564,
        footer: {
          text: "Powered By XD - PROJECT",
          icon_url:
            "https://media.discordapp.net/attachments/1113493409253044386/1113493458947145798/LOGO_1.png?ex=65ba6b18&is=65a7f618&hm=4c764a10bfdef9f37348dfbb9535053fb68d93602d63a49875b9309a6c003fca&=&format=webp&quality=lossless&width=676&height=676",
        },
  ...
  client.channels.cache.get(Room_Discord).send(Messagex);
});
    ...
    embeds: [
      {
        ...
        description:
          "**[ALLRIGHT TOWN](https://www.facebook.com/AllrightTown) ยินดีต้อนรับค้าบ**\nพบปัญหากด <#1063622115447803924> ได้เลย",
        color: 5106515,
        thumbnail: {
  ...
  if (interaction.isButton()) {
    if (interaction.customId == "customer_reaction") {
      const hasRole = interaction.member.roles.cache.some(
        (r) => r.name === "OBT WHITELIST"
      );
      if (hasRole) {
        await interaction.reply({
          content: "คุณมียศอยู่แล้ว!!",
          ephemeral: true,
        });
        return;
      }
